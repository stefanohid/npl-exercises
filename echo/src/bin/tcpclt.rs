use std::{io::{Write}, net::{SocketAddr, TcpStream}, io::Read};
// use std::net::ToSocketAddrs;
use clap::Parser;


#[derive(Parser, Debug)]
//Defines the command line arguments for the TCP client using the clap crate. 
...
//        )).unwrap();
    ...
    verbose_log(cli.verbose, 1, format!("TCP client connected to {}:{}", local_socket.ip(), local_socket.port()));
    ...
    loop {
        ...
        // Note: send_to() is not a method of TcpStream, but we can use write() instead, since TCP is a 
        // stream-oriented protocol and does not require specifying the destination address for each message.
        // The server will receive the data from the client through the established connection, and it can
        // read the data using read() or similar methods.
